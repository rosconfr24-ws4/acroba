...
import acroba_unity_msgs.msg  # skills
from acroba_unity_msgs.srv import GetImage, GetImageResponse
import std_srvs.srv
...
class CameraControl(object):
    ...
    def __init__(self,global_namespace="",local_namespace='/unity_camera'):
        ...
        self._unityclient = actionlib.SimpleActionClient(global_namespace+local_namespace, acroba_unity_msgs.msg.RGBDSensorAction)
        self._subinfo=rospy.Subscriber(global_namespace+local_namespace+'/camera_info',CameraInfo,self.info_callback)
        self.connected = self._unityclient.wait_for_server(rospy.Duration(30))
    ...
    def depthmap_to_pointlist(self,depth_map):
        ...
        for row in range(depth_map.shape[0]):
            for col  in range(depth_map.shape[1]):
                x=depth_map[row,col]*(col-self.intrinsic_matrix[0,2])/self.intrinsic_matrix[0,0]
                y=depth_map[row,col]*(row-self.intrinsic_matrix[1,2])/self.intrinsic_matrix[1,1]
