...
class MoveGripperVG:
    ...
    feedback = skills_msgs.msg.MoveGripperVGFeedback()
    result = skills_msgs.msg.MoveGripperVGResult()

    def __init__(self, name):
        self._action_name = name
        self._as = actionlib.SimpleActionServer(
            self._action_name,
            skills_msgs.msg.MoveGripperVGAction,
    ...
    def execute_cb(self, goal):
        ...
        if parallel_yaw.connected:
            parallel_yaw.open_custom(goal.fingers_poses, goal.width, goal.speed)
...
if __name__ == "__main__":
    rospy.init_node("MoveGripperVG")
    server = MoveGripperVG(rospy.get_name())
    rospy.spin()
