...
"""
Primitive that controls the opening of the Virtual Gym gripper (robotiq Hand-e)
"""
import rospy
import actionlib
...
class ReleaseVG:
    """
    Class for Release VG primitive
    """

    # create messages that are used to publish feedback/result
    feedback = skills_msgs.msg.ReleaseVGFeedback()
    result = skills_msgs.msg.ReleaseVGResult()

    def __init__(self, name):
        self._action_name = name
        self._as = actionlib.SimpleActionServer(
            self._action_name,
            skills_msgs.msg.ReleaseVGAction,
    ...
    def execute_cb(self, goal):
        ...
        if parallel_yaw.connected:
            parallel_yaw.open_full(goal.speed)
...
if __name__ == "__main__":
    rospy.init_node("ReleaseVG")
    server = ReleaseVG(rospy.get_name())
    rospy.spin()
